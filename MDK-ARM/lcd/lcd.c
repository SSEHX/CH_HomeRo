...
uchar lcd_cmd(char *cmd, char *reply,uint const waittime )
{
	...
	LCD_DELAY(waittime);                 //延时
	
	
	return lcd_cmd_check(reply);    //对接收数据进行处理
}
...
uchar lcd_cmd_check(char *reply)
{
	...
	*(redata+len) = '\0';
	...
	return LCD_FALSE;       //跳出循环表示比较完毕后都没有相同的数据，因此跳出
}
...
/************************************************************************/
/* style :	0 此BTN不带框，即为纯文本，等同于DC和DCV指令
1 此BTN带下压框，模拟按钮按下的效果
2 此BTN带抬起框，模拟按钮抬起的效果
4 此BTN带frame_color指定的颜色框
8 此BTN没有底色
*/
/************************************************************************/
